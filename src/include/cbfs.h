...
static inline size_t cbfs_unverified_area_type_load(const char *area, const char *name,
						    void *buf, size_t size, enum cbfs_type *type);
...
static inline void *cbfs_unverified_area_alloc(const char *area, const char *name,
					       ...
					       size_t *size_out)
{
	return _cbfs_unverified_area_type_alloc(area, name, allocator, arg, size_out, NULL);
}
...
static inline void *cbfs_unverified_area_type_map(const char *area, const char *name,
					      size_t *size_out, enum cbfs_type *type)
{
	return _cbfs_unverified_area_type_alloc(area, name, NULL, NULL, size_out, type);
}

static inline void *cbfs_unverified_area_map(const char *area, const char *name,
					     size_t *size_out)
{
	return cbfs_unverified_area_type_map(area, name, size_out, NULL);
}
...
{
	struct _cbfs_default_allocator_arg arg = { .buf = buf, .buf_size = size };
	if (_cbfs_unverified_area_type_alloc(area, name, _cbfs_default_allocator, &arg, &size,
					     type))
		return size;
	else
		return 0;
...
static inline size_t cbfs_unverified_area_load(const char *area, const char *name,
					       void *buf, size_t size)
{
	return cbfs_unverified_area_type_load(area, name, buf, size, NULL);
}
